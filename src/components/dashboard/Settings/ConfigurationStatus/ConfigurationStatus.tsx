...
const ConfigurationStatus: React.FC<ConfigurationStatusProps> = ({ className }) => {
  ...
  const getSandboxIcon = (sandbox: boolean) => {
    if (sandbox) {
      return <AlertTriangle size={20} className="config-status__icon--warning" />;
    }
    return <Check size={20} className="config-status__icon--success" />;
  };
    ...
    <div className={`config-status ${className || ''}`}>
      ...
      <div className="config-status__items">
        <div className="config-status__item">
          ...
          <span className={`config-status__value ${config.configured ? 'success' : 'error'}`}>
        ...
        <div className="config-status__item">
          ...
          <span className={`config-status__value ${config.sandbox ? 'warning' : 'success'}`}>
        ...
        {config.missing.length > 0 && (
          <div className="config-status__missing">
            <h4 className="config-status__missing-title">Missing Environment Variables:</h4>
            ...
            <p className="config-status__missing-help">
              Add these variables to your <code>.env.local</code> file with values from your Autopay merchant dashboard.
            </p>
          </div>
        )}
      </div>
      ...
      {config.configured && (
        <div className="config-status__success">
          <p className="config-status__success-message">
            ...
            {config.sandbox && ' You are in sandbox mode - perfect for testing!'}
          </p>
        </div>
      )}
    </div>
