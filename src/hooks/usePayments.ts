...
export const usePayments = (user?: User) => {
  ...
  const setSuccess = useCallback((payment?: AutopayPaymentResponse, subscription?: AutopaySubscription) => {
    setState(prev => ({ 
      ...prev, 
      success: true, 
      isLoading: false, 
   ...
   */
  const initiatePlanPayment = useCallback(async (
    planId: string,
    planName: string,
    amount: number,
    ...
    try {
      const paymentData: AutopayPaymentRequest = {
        amount,
        currency: 'PLN',
        title: `Plan ${planName} - ${billingCycle === 'monthly' ? 'miesięczny' : 'roczny'}`,        description: `Subskrypcja planu ${planName} dla użytkownika ${user.email}`,
        customerId: user.id,
      ...
      const paymentResponse = await autopayService.createPayment(paymentData, user);
  ...
  const createSubscription = useCallback(async (
    ...
    try {
      const paymentData: AutopayPaymentRequest = {
        amount,
        currency: 'PLN',
        title: `Subskrypcja ${planName}`,        description: `Cykliczna subskrypcja planu ${planName}`,
        customerId: user.id,
        customerEmail: user.email,
        customerName: `${user.firstName} ${user.lastName}` || user.email,
        planId,
      ...
      const subscription = await autopayService.createSubscription(paymentData, user);
  ...
  const checkPaymentStatus = useCallback(async (transactionId: string) => {
    ...
    try {
      const paymentStatus = await autopayService.getPaymentStatus(transactionId);
      ...
      if (paymentStatus.status === 'confirmed') {
        ...
        setState(prev => ({ ...prev, currentPayment: paymentStatus, isLoading: false }));
      }
