...
const PaymentReturn: React.FC = () => {
  ...
  useEffect(() => {
    const handlePaymentReturn = async () => {
      try {
        ...
        setPaymentDetails(paymentStatus);        switch (paymentStatus.status) {
          case 'confirmed':
            setStatus('success');
  ...
  const getStatusIcon = () => {
    switch (status) {
      ...
      case 'cancelled':
        return <AlertCircle size={64} className="payment-return__icon--warning" />;
      case 'processing':
      default:
        return <Loader size={64} className="payment-return__icon--processing" />;
    }
  };
        ...
        <div className="payment-return__card">
          ...
          {paymentDetails && (
            <div className="payment-return__details">
              ...
              {paymentDetails.created_at && (
                <div className="payment-return__detail-row">
                  <span>Data:</span>
                  <span>{new Date(paymentDetails.created_at).toLocaleString('pl-PL')}</span>
                </div>
              )}
            </div>
          )}
          ...
          <div className="payment-return__actions">
            ...
            {status !== 'processing' && status !== 'success' && (
              <button
                onClick={() => navigate('/dashboard/settings', { state: { tab: 'plan' } })}
