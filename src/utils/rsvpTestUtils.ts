...
export class RSVPTestUtils {
   ...
   */
  static async testTokenGeneration(guestId: string, eventId: string): Promise<{
    success: boolean;
    token?: string;
    error?: string;
  }> {
  ...
  static testEmailConfiguration(): {
    ...
    if (emailStatus.configured) {
      ...
      recommendations.push('Add EmailJS environment variables to enable email sending');
   ...
   */
  static async testEmailSending(mockInvitation?: Partial<GuestInvitation>, mockEvent?: Partial<Event>): Promise<{
    success: boolean;
    error?: string;
  }> {
    try {
   ...
   * Run all RSVP tests
   */
  static async runAllTests(guestId?: string, eventId?: string): Promise<{
    tokenTest: { success: boolean; token?: string; error?: string };
    emailConfigTest: { configured: boolean; status: string; recommendations: string[] };
    ...
    if (guestId && eventId) {
      ...
      console.log('⏭️ Skipping token generation test (no guest/event IDs provided)');
